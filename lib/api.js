...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // ✅ Sirf tab redirect karo jab login page pe na ho (loop fix)
...
export const realEstateAPI = {
  ...
  properties: {
    ...
    create: (formData) => {
      return api.post('/real-estate/properties', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
    },
    update: (id, formData) => {
      return api.put(`/real-estate/properties/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
    },
    close: (id, data) => api.patch(`/real-estate/properties/${id}/close`, data),
  ...
  // ============================================================
  requirements: {
    getAll: (params) => api.get('/real-estate/requirements', { params }),
    getById: (id) => api.get(`/real-estate/requirements/${id}`),
    create: (data) => api.post('/real-estate/requirements', data),
    update: (id, data) => api.put(`/real-estate/requirements/${id}`, data),
    delete: (id) => api.delete(`/real-estate/requirements/${id}`),
  },
  ...
  // ============================================================
  revenue: {
    get: () => api.get('/real-estate/revenue'),
    getTransactions: () => api.get('/real-estate/revenue/transactions'),
  },
};
